...
import { obtenerCategorias, obtenerEquipo, actualizarEquipo, subirImagen } from './servidor'
...
export default function EditarEquipo() {
  ...
  const cargarDatos = async () => {
    try {
      setLoading(true)
      const [dataCategorias, dataEquipo] = await Promise.all([
        obtenerCategorias(),
      ...
      let caracteristicas = []
      if (dataEquipo.caracteristicas) {
        try {
          caracteristicas = typeof dataEquipo.caracteristicas === 'string' 
            ? JSON.parse(dataEquipo.caracteristicas) 
            : dataEquipo.caracteristicas
        } catch (e) {
          caracteristicas = []
        }
      }
      ...
      setFormData({
        categoria_id: dataEquipo.categoria_id || '',
        codigo: dataEquipo.codigo || '',
        nombre: dataEquipo.nombre || '',
        tipo: dataEquipo.tipo || '',
        marca: dataEquipo.marca || '',
        modelo: dataEquipo.modelo || '',
        capacidad: dataEquipo.capacidad || '',
        descripcion_corta: dataEquipo.descripcion_corta || '',
        descripcion_completa: dataEquipo.descripcion_completa || '',
        tecnologia: dataEquipo.tecnologia || '',
        caracteristicas: caracteristicas,
        estado: dataEquipo.estado || 'disponible',
        stock: dataEquipo.stock || 1,
        precio_dia: dataEquipo.precio_dia || '',
        precio_semana: dataEquipo.precio_semana || '',
        precio_mes: dataEquipo.precio_mes || '',
        precio_compra: dataEquipo.precio_compra || '',
        fecha_adquisicion: dataEquipo.fecha_adquisicion ? dataEquipo.fecha_adquisicion.split('T')[0] : '',
        garantia_meses: dataEquipo.garantia_meses || '',
        peso_kg: dataEquipo.peso_kg || '',
        dimensiones: dataEquipo.dimensiones || '',
        imagen_principal: dataEquipo.imagen_principal || '',
        activo: dataEquipo.activo,
        destacado: dataEquipo.destacado
      })

      setImagenPrincipalPreview(dataEquipo.imagen_principal)
      setImagenes(dataEquipo.imagenes || [])
      setEspecificaciones(dataEquipo.especificaciones || [])
    } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await actualizarEquipo(params.id, datosEquipo)
      ...
      mostrarMensaje(error.message || 'Error al actualizar equipo', 'error')
    } finally {
      setProcesando(false)
    }
  }
    ...
    <div className={styles.container}>
      <div className={styles.header}>
        <div className={styles.headerContent}>
          ...
          <p className={styles.subtitle}>Actualiza los datos del equipo</p>
        </div>
        <button 
          type="button"
      ...
      <form onSubmit={handleSubmit} className={styles.form}>
        ...
        <div className={styles.formFooter}>
          ...
          >
            {procesando ? 'Guardando...' : 'Actualizar Equipo'}
          </button>
        </div>
      </form>
    </div>
